...
class NodeSelector:
    ...
    def __select_fastest(self, state: NodeSelector.__NodeSelectorState, index: int) -> None:
        ...
        if self.__update_fastest_node_timer is not None:
            self.__update_fastest_node_timer.change(datetime.timedelta(minutes=1))

        else:
            self.__update_fastest_node_timer = Timer(
                self.__switch_to_speed_test_phase, datetime.timedelta(minutes=1), self.__thread_pool_executor
            )
