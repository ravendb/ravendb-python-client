...
from pyravendb.legacy.commands.raven_commands import (
    CreateSubscriptionCommandOld,
    DeleteSubscriptionCommandOld,
    GetSubscriptionsCommandOld,
    GetSubscriptionStateCommandOld,
    DropSubscriptionConnectionCommandOld,
)
from threading import Lock

from pyravendb.legacy.subscriptions.data import SubscriptionCreationOptions, SubscriptionWorkerOptions
from pyravendb.legacy.subscriptions.subscription import SubscriptionWorker
...
class DocumentSubscriptions:
    ...
    def _create(self, options, database=None):
        ...
        command = CreateSubscriptionCommandOld(options)
    ...
    def delete(self, name_or_id, database=None):
        request_executor = self._store.get_request_executor(database)
        command = DeleteSubscriptionCommandOld(name_or_id)
    ...
    def drop_connection(self, name, database=None):
        ...
        """
        request_executor = self._store.get_request_executor(database)
        command = DropSubscriptionConnectionCommandOld(name)
        request_executor.execute(command)
    ...
    def get_subscriptions(self, start, take, database=None):
        ...
        """
        request_executor = self._store.get_request_executor(database)
        command = GetSubscriptionsCommandOld(start, take)
        return request_executor.execute(command)
    ...
    def get_subscription_state(self, subscription_name, database=None):
        request_executor = self._store.get_request_executor(database)
        command = GetSubscriptionStateCommandOld(subscription_name)
        return request_executor.execute(command)
