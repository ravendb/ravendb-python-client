...
from pyravendb.legacy.commands.raven_commands import GetTcpInfoCommandOld
...
from pyravendb.exceptions.exceptions import AggregateException
import ijson
...
class SubscriptionWorker:
    def __init__(
        ...
        if options.subscription_name is None:
            raise AttributeError(
                "SubscriptionWorkerOptions must specify the subscription_name",
                "options",
            )
    ...
    def connect_to_server(self):
        command = GetTcpInfoCommandOld("Subscription/" + self._database_name, self._database_name)
        ...
        self.request_executor = RequestExecutor.create_for_single_node(
            command.requested_node.url, self._database_name, self._store.certificate
        )
            ...
            try:
                ...
                try:
                    self._logger.warning(
                        "Subscription '{0}'. Pulling task threw the following exception : {1}".format(
    ...
    def should_try_to_reconnect(self, exception):
        if isinstance(exception, SubscriptionDoesNotBelongToNodeException):
            ...
            node_to_redirect_to = Utils.first_or_default(
                request_executor.topology_nodes,
        ...
        if isinstance(
            ...
            (
                ...
                NonRecoverableSubscriptionException,
                ...
                NotImplementedError,
                AttributeError,
                ijson.backend.UnexpectedSymbol,
            ),
        ):
            self.close()
            return False
    ...
    def assert_connection_state(self, connection_status):
        ...
        if connection_status["Type"] != "ConnectionStatus":
            raise Exception(
                "Server returned illegal type message when expecting connection status, was: "
    ...
    def run(self, process_documents):
        ...
        self._subscription_thread = Thread(
            target=self.run_subscription
        )  # todo: restore PropagatingThread, look history
        ...
        for item in self.raw_items:
            ...
            self.items.append(
                _SubscriptionBatchItem(
                    change_vector=last_change_vector,
