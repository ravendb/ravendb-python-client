...
class TimeSeries:
    ...
    def raise_not_in_session(entity):
        raise ValueError(
            repr(entity)
            + " is not associated with the session, cannot add time-series to it. "
    ...
    def _add_to_cache(self, ranges, from_date, to_date, start, page_size):
        ...
        for range_ in ranges:
            if range_.from_date <= start_date:
                ...
                if range_.from_date <= to_date:
                    time_series_ranges.insert(
                        index, TimeSeriesRange(self._name, start_date, range_.from_date)
                    )
                    index += 1
                    ...
                    time_series_ranges.insert(
                        index, TimeSeriesRange(self._name, start_date, to_date)
                    )
                    start_date = None
                    break
        if start_date:
        ...
        details = self._session.advanced.document_store.operations.send(
            GetTimeSeriesOperation(
                self._document_id, time_series_ranges, start, page_size
            )
        )

        results = [
            TimeSeriesRangeResult.create_time_series_range_entity(i)
            for i in details["Entries"]
        ]
        ...
        for range_ in ranges:
            ...
            if (
                ...
                if time_series_range_result is None:
                    time_series_range_result = TimeSeriesRangeResult(
                        None, None, entries=[]
                    )
                ...
                if not start_from:
                    start_from = (
                        from_date if from_date < range_.from_date else range_.from_date
                    )
                if not end_from:
        ...
        if time_series_range_result:
            ...
            time_series_range_result.from_date = (
                start_from
                if start_from < time_series_ranges[0].from_date
                else time_series_ranges[0].from_date
            )
            time_series_range_result.to_date = (
                end_from
                if end_from > time_series_ranges[0].to_date
                else time_series_ranges[-1].to_date
            )
    ...
    def get(
        ...
        if ranges_result:
            if (
                ranges_result[0].from_date > to_date
                or ranges_result[-1].to_date < from_date
            ):
                # the entire range [from_date, to_date] is out of cache bounds
                self._session.increment_requests_count()
                ranges = TimeSeriesRange(
                    name=self._name, from_date=from_date, to_date=to_date
                )
                details = self._session.advanced.document_store.operations.send(
                    GetTimeSeriesOperation(self._document_id, ranges, start, page_size)
                )

                time_series_range_result = (
                    TimeSeriesRangeResult.create_time_series_range_entity(details)
                )

                if not self._session.no_tracking:
                    index = (
                        0
                        if ranges_result[0].from_date > to_date
                        else len(ranges_result)
                    )
            ...
            entries_to_return, cache_ranges = self._add_to_cache(
                ranges_result, from_date, to_date, start, page_size
            )
            if not self._session.no_tracking:
        ...
        if not self._session.no_tracking and entries:
            if cache:
                ...
                self._session.time_series_by_document_id[self._document_id] = {
                    self._name: [time_series_range_result]
                }
    ...
    def append(
        ...
        if not isinstance(values, list):
            values = [values]
        command = self._session.timeseries_defer_commands.get(
            (self._document_id, "TimeSeries", self._name), None
        )
        if command:
            ...
            command = TimeSeriesBatchCommandData(
                self._document_id, self._name, operation=operation
            )
            self._session.timeseries_defer_commands[
                (self._document_id, "TimeSeries", self._name)
            ] = command
