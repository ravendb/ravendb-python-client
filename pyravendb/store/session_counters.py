...
class DocumentCounters:
    ...
    def increment(self, counter_name, delta=1):
        ...
        command: CountersBatchCommandData = self._counters_defer_commands.get(self._document_id, None)
        if command:
            ...
            self._counters_defer_commands[self._document_id] = command
    ...
    def get(self, counter_names: List[str] or str):
        ...
        for counter_name in counter_names:
            val = cache.get(counter_name, None)
            if val and metadata_counters and counter_name in metadata_counters:
            ...
            for counter_detail in details["Counters"]:
                cache[counter_detail["CounterName"]] = counter_detail["TotalValue"]
