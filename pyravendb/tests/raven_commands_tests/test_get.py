...
class TestGet(TestBase):
    def setUp(self):
        ...
        put_command = PutDocumentCommand(
            "products/101", {"Name": "test", "@metadata": {}}
        )
        put_command2 = PutDocumentCommand(
            "products/10", {"Name": "test", "@metadata": {}}
        )
        put_command3 = PutDocumentCommand(
            "products/102", {"Name": "test", "@metadata": {}}
        )
        ...
        self.response = self.requests_executor.execute(
            GetDocumentCommand("products/101")
        )
        self.other_response = self.requests_executor.execute(
            GetDocumentCommand("products/10")
        )
        self.paged_response = self.requests_executor.execute(
            GetDocumentCommand("products/102", start=2, page_size=1)
        )
    ...
    def test_equal(self):
        self.assertEqual(
            self.response["Results"][0]["@metadata"]["@id"], "products/101"
        )
    ...
    def test_paging(self):
        ...
        self.assertEqual(
            self.paged_response["Results"][0]["@metadata"]["@id"], "products/102"
        )
