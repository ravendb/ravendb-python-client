...
class TestQuery(TestBase):
    ...
    def setUpClass(cls):
        ...
        cls.requests_executor.excute(PutDocumentCommand("products/10", {"Name": "test", "@collection": "Products",
                                                                        "Raven-Python-Type": "object"}))

    def test_only_query(self):
        self.requests_executor.execute(PutIndexesCommand(self.index))
        query_command = QueryCommand("Testing", IndexQuery("Tag:Products"))
    ...
    def test_get_only_metadata(self):
        self.requests_executor.execute(PutIndexesCommand(self.index))
        query_command = QueryCommand("Testing", IndexQuery("Tag:Products"), metadata_only=True)
        response = self.requests_executor.execute(query_command)
        self.assertFalse("Name" in response["Results"][0])

    def test_get_only_index_entries(self):
        self.requests_executor.execute(PutIndexesCommand(self.index))
        query_command = QueryCommand("Testing", IndexQuery("Tag:Products"), index_entries_only=True)
        response = self.requests_executor.execute(query_command)
        self.assertFalse("@metadata" in response["Results"][0])

    def test_fail(self):
        with self.assertRaises(ValueError):
            query_command = QueryCommand(None, IndexQuery("Tag:Products"))
    ...
    def test_fail_none_response(self):
        with self.assertRaises(exceptions.ErrorResponseException):
            query_command = QueryCommand("IndexIsNotExists", IndexQuery("Tag:Products"))
