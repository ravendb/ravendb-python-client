...
class TestOperations(TestBase):
    ...
    def test_put_attachment(self):
        ...
        put_result = self.store.operations.send(
            PutAttachmentOperation(
                "users/1-A", "my_picture", binary_list, content_type="image/png"
            )
        )
    ...
    def test_delete_attachment(self):
        ...
        get_attachment_operation = GetAttachmentOperation(
            "users/1-A", "my_picture", AttachmentType.document, None
        )
    ...
    def test_patch_by_index(self):
        ...
        with self.store.open_session() as session:
            ...
            query_result = list(
                session.query(wait_for_non_stale_results=True).raw_query(
                    "FROM INDEX 'Patches' Where patched=False"
                )
            )
            assert query_result
        ...
        operation = PatchByQueryOperation(
            query_to_update=IndexQuery(
                query="FROM INDEX 'Patches' Where patched=False UPDATE {{this.patched=true}}"
            ),
            options=options,
        )
    ...
    def test_fail_patch_wrong_index_name(self):
        ...
        with self.assertRaises(exceptions.InvalidOperationException):
            ...
            if result:
                self.store.operations.wait_for_operation_complete(
                    result["operation_id"]
                )
            else:
                raise exceptions.ErrorResponseException(
                    "Got empty or None response from the server"
                )
    ...
    def test_delete_by_index(self):
        self.store.maintenance.send(
            PutIndexesOperation(
                IndexDefinition(
                    "Users", maps="from doc in docs.Users select new {name=doc.name}"
                )
            )
        )
    ...
    def test_patch_by_collection(self):
        ...
        index_query = IndexQuery(
            "From Patches Update {{this.patched=true}}", wait_for_non_stale_results=True
        )
    ...
    def test_delete_by_collection_(self):
        ...
        operation_id = self.store.operations.send(
            DeleteByQueryOperation(query_to_delete=index_query)
        )["operation_id"]
