...
from ravendb.documents.operations.cdc_sink.configuration import (
...
class AddCdcSinkOperation(MaintenanceOperation[AddCdcSinkOperationResult]):
    """Adds a new CDC Sink task, streaming an SQL source into RavenDB collections."""
    ...
    def get_command(self, conventions: "DocumentConventions") -> RavenCommand[AddCdcSinkOperationResult]:
        return self._AddCdcSinkCommand(self._configuration)

    class _AddCdcSinkCommand(RavenCommand[AddCdcSinkOperationResult], RaftCommand):
        def __init__(self, configuration: CdcSinkConfiguration):
...
class UpdateCdcSinkOperation(MaintenanceOperation[UpdateCdcSinkOperationResult]):
    """Updates an existing CDC Sink task."""
    ...
    def get_command(self, conventions: "DocumentConventions") -> RavenCommand[UpdateCdcSinkOperationResult]:
        return self._UpdateCdcSinkCommand(self._task_id, self._configuration)

    class _UpdateCdcSinkCommand(RavenCommand[UpdateCdcSinkOperationResult], RaftCommand):
        def __init__(self, task_id: int, configuration: CdcSinkConfiguration):
